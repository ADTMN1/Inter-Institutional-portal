...
export default function RequestsPage() {
  ...
  const handleRequestAction = async (action: string, request: any) => {
    ...
    if (action === "details") {
      ...
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/api/requests${request.id}`
        );
  ...
  const handleStopConversation = async (requestId: string) => {
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/requests/${requestId}/stop-conversation`,
        {
          method: "POST",
        }
      );
  ...
  const handleResumeConversation = async (requestId: string) => {
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/requests/${requestId}/resume-conversation`,
        {
          method: "POST",
        }
      );
