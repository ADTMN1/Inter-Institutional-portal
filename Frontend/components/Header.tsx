...
function HeaderContent() {
  ...
  const { notifications, markAsRead, markAsUnread, fetchNotifications } =
    useNotifications();

  const [notificationsState, setNotifications] = useState<any[]>([]);
    ...
    <header className="bg-white border-b border-gray-200 px-6 py-4">
      <div className="flex items-center justify-between w-full">
        ...
        <div className="flex items-center gap-3">
          <button
            onClick={() => setOpenSheet(true)}
            className="relative flex items-center px-2 sm:px-4 py-2 rounded-lg"
            aria-label="Notifications"
          >
            <Bell className="w-6 h-6 sm:w-5 sm:h-5" />
            {!isMobile && <span className="ml-1">Notifications</span>}
            ...
            {unreadCount > 0 && (
              <span className="absolute -top-2 right-2 w-5 h-5 text-xs bg-red-500 text-white rounded-full flex items-center justify-center font-bold">
                {unreadCount}
              </span>
            )}
          </button>
